...
export const AppSequence: React.FC<{
  ...
  return <AbsoluteFill style={{fontFamily, color: colors.ink}}><Grid />{showDisclosure ? <><div style={{position: 'absolute', left: 76, top: 24, color: colors.blue, fontSize: 16, fontWeight: 750, letterSpacing: .7}}>讲解模拟 · 对应当前 Workbench · 公开合成资料</div><div style={{position: 'absolute', right: 76, top: 24, color: colors.muted, fontSize: 14}}>数字案例只解释规则差异 · 产品画面停在候选定义</div></> : null}<div style={{position: 'absolute', left: 76, top: 62, width: 1768, height: 888, display: 'grid', gridTemplateColumns: '252px minmax(0,1fr) 610px', overflow: 'hidden', border: `1px solid ${colors.lineStrong}`, background: '#F7F9FB', boxShadow: '0 32px 86px rgba(19,36,58,.18)', opacity, transform: `scale(${scale})`, transformOrigin: 'center center'}}><Navigation frame={frame} /><main style={{display: 'flex', minWidth: 0, minHeight: 0, flexDirection: 'column', overflow: 'hidden', background: '#F7F9FB'}}><CenterPanel frame={frame} /></main><AgentPanel frame={frame} /></div><Cursor frame={frame} />{showCaption && frame < 1230 ? <Caption>{caption}</Caption> : null}</AbsoluteFill>;
};
