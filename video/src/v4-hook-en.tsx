...
const TABLE = {
  ...
  rowHeight: 100,
  columns: [
    {key: 'order', label: 'Order', x: 0, width: 220},
    {key: 'region', label: 'Region', x: 220, width: 260},
    {key: 'status', label: 'Status', x: 480, width: 316},
    {key: 'amount', label: 'Amount', x: 796, width: 380},
  ],
};
...
const ORDERS = [
  {order: 'O001', region: 'East China', status: 'Paid', amount: 120, state: 'paid'},
  {order: 'O003', region: 'East China', status: 'Refunded', amount: 50, state: 'refunded'},
  {order: 'O004', region: 'East China', status: 'Pending', amount: 199, state: 'pending'},
  {order: 'O006', region: 'East China', status: 'Paid', amount: 320, state: 'paid'},
] as const;
...
const Disclosure: React.FC<{onWorkbench?: boolean}> = ({onWorkbench = false}) => (
  ...
  >
    Public synthetic Demo · the three numbers are not product results
  </div>
);
...
const QuestionHeader: React.FC<{compact?: boolean}> = ({compact = false}) => (
  <>
    ...
    >
      East China · public synthetic orders
    </div>
    <div
      style={{
        position: 'absolute',
    ...
    >
      {compact ? 'One order, three rules' : 'What is the East China total?'}
    </div>
  </>
);
...
const EastOrdersScene: React.FC = () => {
  ...
  const amountColumn = TABLE.columns[3];
  const numberPrompts = [
    {value: '689？', label: 'All statuses', from: 48},
    {value: '440？', label: 'Paid only', from: 56},
    {value: '390？', label: 'Minus refunds', from: 64},
  ];
  return (
...
const PlainLanguageScene: React.FC = () => (
  ...
  >
    ...
    <div style={{fontSize: 82, lineHeight: 1.12, fontWeight: 820, letterSpacing: -3.2}}>
      The math is not wrong; the rule is unclear
    </div>
    <div style={{marginTop: 30, fontSize: 34, color: COLORS.muted}}>
      What counts · how refunds work · which time to use
    </div>
    <div
      style={{
        position: 'absolute',
...
const WorkbenchEntryScene: React.FC<{duration: number}> = ({duration}) => {
  ...
  return (
    <AbsoluteFill style={{background: COLORS.surface}}>
      ...
      >
        Confirm sources, state the goal
      </div>
      <div
        style={{
          position: 'absolute',
...
export const ContextOxProductFilmV4HookAEn: React.FC = () => <ProductFilmV4Hook variant="A" />;

export const ContextOxProductFilmV4HookBEn: React.FC = () => <ProductFilmV4Hook variant="B" />;

export const ContextOxProductFilmV4HookAVisualEn: React.FC = () => (
