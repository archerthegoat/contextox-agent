...
const rawPath = resolve(outputRoot, 'contextox-demo-1.0.0-product-film-v4-a-raw.mp4');
const finalPath = resolve(outputRoot, 'contextox-demo-1.0.0-product-film-v4-a-candidate.mp4');
...
if (!normalizeOnly) {
  ...
  throw new Error(`Raw V4 render not found: ${rawPath}`);
}
...
const analysis = run(
  ...
  {capture: true, env: ffmpegEnvironment},
).stderr;

const match = analysis.match(/\{\s*"input_i"[\s\S]*?\}/);
if (!match) throw new Error('Could not read loudnorm analysis from the raw render.');
...
if (!existsSync(finalPath)) throw new Error('Normalized V4 candidate was not created.');
rmSync(rawPath);
