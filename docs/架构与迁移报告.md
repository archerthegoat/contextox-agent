...
- 新 Run 引用获批 Contract、满足任务模板的收据和证据后，显式完成事务才能令 Mission `completed`；
...
候选表：`workspaces`、`source_bindings`、`source_revisions`、`source_artifacts`、`missions`、`agent_runs`、`public_run_events`、`clarification_requests`、`clarification_answers`、`approval_requests`、`decisions`、`contract_versions`、`workspace_context_versions`、`context_version_sources`、`tool_receipts`、`audit_events`。`public_run_events` 不逐条保存 `model_delta`。
