...
### 0.6 Agent 主导 Workbench 实施检查点（2026-09-10）

需求来源：本任务中用户明确发出的“PLEASE IMPLEMENT THIS PLAN: 数契 Workbench：由 Agent 主导的交互重设计”。用户授权按原型、连续会话、受限讨论、回答交接、工作台接入顺序完成本地实现；方案同时保留两份受控文档的精确修订批准前置。本节只记录该批执行与证据，不替代开发路径图或架构合同。

- 基线：干净的 `main@b92118be82aaf9fac5bdaaa18d6947e7e08adafd`，当时与 `origin/main` 无分歧。本地主分支为 `codex/agent-led-workbench`；无 push、main merge、发布或真实 Provider 调用。
- 原型独立写入 worktree：`/private/tmp/contextox-agent-led-prototype`，分支 `codex/agent-led-workbench-prototype`。原型提交 `49aeb7273302dc93b280fe1ef167abf0761b2a50` 已以 `b6fe76b` 集成本地主分支。主任务拥有合同、审批补丁和执行记录；独立只读审查不写共享合同。
- 合成原型入口：[README](prototypes/agent-led-workbench/README.md)、[页面](prototypes/agent-led-workbench/index.html)。仅演示固定公开合成流程；不读取文件正文、不接收真实密钥、不连接模型。宽度偏好保留在本机，原型对话刷新会重置，不能作为连续会话持久化证据。
- 已准备 [实施合同](proposals/Agent主导Workbench-实施合同.md) 及 [受控文档精确修订](proposals/Agent主导Workbench-受控文档待批准.diff)。合同 SHA256 为 `006ff8930e05464d7a1be76f564f9a08bd8eb18ea4769ea9adcafaaff65ee34f`，补丁中的两个采用引用匹配；`git apply --check` 通过。实际开发路径图与架构报告保持未修改，批准状态 `PENDING`。
- 审查补齐三处边界并复核：启动短句必须关联此前可见业务目标及用户消息来源；讨论转分析保留同一活动槽并以收据、用量、取消和版本校验衔接；回答交接固定整份已核对版本及子请求身份，不在重试时改用最新未审版本。该审查通过不代表受控文档已经获批。
- 后续依赖工作：获准后只应用精确补丁，再按合同推进会话/显式迁移、任务创建、回答交接和真实 Workbench 接入，各片运行必要检查并本地提交。真实模型案例与预算、人的产品验收仍分别等待独立授权和结果。

浏览器验证使用本地静态原型服务，覆盖 1440×900、775×800 与 390×844：任务前讨论、发送推进到分析、澄清解释与卡片编辑、仅保存、确认续接、手动查看保护和恢复跟随、指针/键盘调宽与重载偏好、窄屏对话/工作区切换、运行中编辑及停止保留草稿、仅保存连接设置与保存后发送、批准后启动失败恢复、未知结果核对及展开/恢复双栏。窄屏页面宽度未超出视口，输入区及发送按钮可达。检查发现的保存状态标签与场景选择不同步、切换场景消息滚动不稳定，已由子提交 `b4477f6` 修复并以 `33f2989` 集成；根代理浏览器复核 390px 下滚动抵达消息底部、保存显示已保存、修改生成版本 2 并恢复待确认、停止后场景显示已停止。JS 语法与 diff 检查通过。

证据分层：静态原型及上述浏览器合成流程已有证据；后端迁移、真实持久消息、生产幂等及接口漂移检查 `NOT RUN`；真实模型验证 `NOT RUN`；人工验收 `PENDING`；用户价值 `NOT VERIFIED`。回退本切片只需撤销本批原型/提案/记录提交，未改变运行时数据库、依赖或既有服务。原型预览仍使用子 worktree，保留其分支和目录以供本地审阅。
