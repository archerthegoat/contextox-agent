...
def enabled(connection):
    return connection.execute("PRAGMA user_version").fetchone()[0] in {5, 6, 7}
        ...
        for item in approved.answer.items:
            ...
            for target in item.targets:
                ...
                for value in merged.values():
                    if getattr(value,key) == target.key and (getattr(value,target.property) is not None or not any(
                        (u.property_path.removeprefix(f"{target.key}.") if target.kind == "relationship"
                         else u.property_path) == target.property for u in value.unknowns)):
