...
候选草案允许保留待补充维度，当前问题无需覆盖所有未知项；正式提交审阅仍执行完整校验。Demo 对已完整返回的 JSON/Schema 格式错误至多纠正一次，两次请求共享 75 秒 Run 上限，单次 Provider 仍至多 70 秒。纯生成失败且已确认未进入领域写入时，任务对话提供“重新生成并发送”；保留原未知回执，不自动重试。短 @ 引用点击后从本地 Store 读取精确来源版本与片段。
