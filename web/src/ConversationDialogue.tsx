...
export function ConversationDialogue({state,d,onReference,onSources,onHistory,onResults,onClarifications,onDemoLoaded}: {
  ...
  const answerPrompt=`请逐项回答这些业务问题，并说明回答人和依据：${state.clarifications.flatMap(item=>item.questions.map(question=>question.question)).join("；")}`;
  const nextActions = nextStep === "active" ? null : <section className="conversation-next-actions" aria-label="建议下一步">
    ...
    {nextStep === "answer" && <><strong>回答 {questionCount} 个会改变结果的问题</strong><div><button className="path2-primary-button" type="button" onClick={()=>focusComposer(answerPrompt)}>用对话回答</button><button type="button" onClick={()=>{onClarifications();answerCards.current?.scrollIntoView({block:"start"});}}>填写回答卡片</button><button type="button" disabled={blocked} onClick={()=>void d.submit(`请解释为什么需要确认这些业务问题，尤其是：${state.clarifications[0]?.questions[0]?.question ?? "当前问题"}`)}>为什么需要确认</button></div></>}
