...
export function ModelSettings() {
  ...
  const unavailable = loading || pending || !settings || settings.busy || settings.source === "environment";
  return <>
    ...
    <dialog ref={dialog} className="workbench-dialog" aria-labelledby="model-settings-title"
      ...
      {loading ? <p role="status">读取本机配置…</p> : settings && <>
        <p className="settings-status">{({environment:"已配置 · 由启动环境管理",keychain:"已配置 · 保存在 macOS Keychain",missing:"尚未配置 Key",unavailable:"暂时无法访问 Keychain"})[settings.source]}</p>
        <p className="settings-detail">DeepSeek Flash · {settings.thinking === "disabled" ? "Demo 快速模式（非思考）" : "生产模式（high）"}。保存状态不代表已验证模型连接。</p>
        {settings.source === "environment" && <p>环境变量优先于 Keychain。请在启动环境修改或移除 Key，再重启服务。</p>}
